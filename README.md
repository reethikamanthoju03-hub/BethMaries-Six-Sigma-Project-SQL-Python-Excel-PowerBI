...
**SQL Analysis:** [`data/Beth_Maries_sql.sql.html`](data/Beth_Maries_sql.sql.html)
...
**Dataset:** 42 in-person customer observations (decision time, scoop time, checkout time, item type) collected on-site at Beth Marie's on the Square, expanded via Excel-based Monte Carlo simulation into a 5,000-customer dataset for reliable statistical analysis, plus a customer perception survey.
...
## Analysis
Metrics calculated and evaluated (see `data/Beth_Maries_sql.sql.html`, `data/BethMaries_Six_Sigma_Dashboard.xlsx`, and the two PDF reports):
...
**4. SQL Analysis** — Loaded the simulated dataset into a relational table and used SQL (CTEs, window functions, z-score outlier detection, `RANK`/`PERCENT_RANK`) to quantify KPIs, flag statistical outliers, and model a peak-hour capacity scenario (see `data/Beth_Maries_sql.sql.html`).
...
- Run the queries in [`data/Beth_Maries_sql.sql.html`](data/Beth_Maries_sql.sql.html) against the "Simulated Data (5000)" sheet loaded into MySQL/SQL Server/Postgres
