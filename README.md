...
This project applies Six Sigma DMAIC methodology to decrease waiting times and product delivery discrepancies at Beth Marie's on the Square which is a famous ice cream store located in Denton, TX. It will cover aspects like the process mapping, time study data collection in person, and applying Monte Carlo simulations for any improvement suggestions based on the  data.
...
| `sql/bethmaries_analysis.sql` | KPI, outlier-detection, and capacity queries — CTEs, window functions, z-scores |
...
**Excel Dashboard:** `excel/BethMaries_Six_Sigma_Dashboard.xlsx`
**Power BI Dashboard:** `powerbi/BethMaries_Dashboard.pbix`
**SQL Analysis:** `sql/bethmaries_analysis.sql`
**Full DMAIC Report:** `reports/Final_Report_4700_Beth_Maries.pdf`
...
## Analysis
Metrics calculated and evaluated (see `sql/bethmaries_analysis.sql`, `excel/BethMaries_Six_Sigma_Dashboard.xlsx`, and the two PDF reports):
...
**4. SQL Analysis** — Loaded the simulated dataset into a relational table and used SQL (CTEs, window functions, z-score outlier detection, `RANK`/`PERCENT_RANK`) to quantify KPIs, flag statistical outliers, and model a peak-hour capacity scenario (see `sql/bethmaries_analysis.sql`).
...
- Run the queries in `sql/bethmaries_analysis.sql` against the "Simulated Data (5000)" sheet loaded into MySQL/SQL Server/Postgres
