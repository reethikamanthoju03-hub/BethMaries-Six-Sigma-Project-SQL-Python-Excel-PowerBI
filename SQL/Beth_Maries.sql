...
-- Run against the Simulated Data (5,000-customer Monte Carlo dataset)
-- Compatible with PostgreSQL / SQL Server / MySQL (minor syntax notes below)
-- =====================================================================

-- ---------------------------------------------------------------------
-- 0. TABLE SETUP
-- Load "Simulated Data (5000)" from BethMaries_Simulated_5000.xlsx into
-- this table (or point your BI tool / SQL import wizard at the same sheet).
-- ---------------------------------------------------------------------
CREATE TABLE service_observations (
    ...
    total_service_time_s DECIMAL(6,2) NOT NULL, -- scoop_time_s + checkout_time_s
...
-- Mirrors the "Total Revenue / Orders / AOV" style KPI block, adapted to
-- service-time KPIs: mean, median, std dev per stage.
-- =====================================================================
SELECT
    ...
    ROUND(AVG(total_service_time_s), 2)     AS avg_total_service_time,
    ROUND(STDDEV(total_service_time_s), 2)  AS stddev_total_service_time,
...
-- Standardizes variability across stages with different means — shows
-- decision time is not just slowest on average, it's also least predictable.
-- =====================================================================
SELECT
...
-- Window functions flag any customer whose scoop time is 2+ standard
-- deviations above the mean (a simple statistical process control signal).
-- =====================================================================
WITH stats AS (
    SELECT
...
-- Useful for the Improve phase — quantifies how bad the "long tail" is.
-- =====================================================================
SELECT
    customer_id,
    item_selected,
...
-- Models the "what if we cut average decision time by 15 seconds" scenario
-- referenced in the Improve phase.
-- =====================================================================
SELECT
    ROUND(AVG(total_service_time_s + decision_time_s), 2)                AS current_avg_full_service_time,
    ROUND(AVG(GREATEST(decision_time_s - 15, 0) + total_service_time_s), 2) AS projected_avg_full_service_time,
    ROUND(
        100.0 * (
...
-- Compares average total service time against observed peak arrival rate
-- to flag hours where required parallel service lines > 1.
-- =====================================================================
SELECT
    day_of_week,
    time_window,
    ...
    (SELECT ROUND(AVG(total_service_time_s), 1) FROM service_observations) AS avg_service_time_s,
    CEIL(
        (SELECT AVG(total_service_time_s) FROM service_observations)
...
-- Flags item categories with too few observations for reliable inference —
-- supports the recommendation to expand the real-world sample.
-- =====================================================================
SELECT
    item_selected,
    COUNT(*) AS n_orders,
...
-- PERCENT_RANK()/RANK() are ANSI-standard window functions, supported by all file three.
